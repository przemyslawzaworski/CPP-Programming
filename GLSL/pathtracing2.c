// cl pathtracing2.c opengl32.lib user32.lib gdi32.lib
...
static const char* BufferA = \
	...
	"float map(vec3 p)"
	"{"  
		"return max(-box(p - vec3(0, 0, 3), vec3(1.0)), max(box(p, vec3(3, 2, 3)), -box(p, 0.75 * vec3(3, 2, 3))));"
	"}"
	...
	"{"
		...
		"vec3 sunDir = normalize(vec3(100,150.0,150.0));"
